...
const baseHtmlPath = path.join(__dirname, 'resource', 'prompt-detail.html');
...
const indexHtmlPath = path.join(__dirname, 'resource', 'index.html');
...
단, 너무 기계적이거나 뻔한 소제목("최종 결론 및 액션 플랜" 등)은 피하고, 사람들의 클릭을 유도할 수 있는 매력적인 카피라이팅으로 소제목을 작성해.</div>`;
  ...
  fs.writeFileSync(path.join(__dirname, 'resource', page.filename), html);
});
