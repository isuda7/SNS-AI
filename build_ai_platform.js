...
// The new AI Platform Sidebar Navigation String
...
// Helper function to update a file's sidebar
function updateSidebarInFile(content, mapping = null) {
  // Find the old AI Platform block
...
// 1. Generate the 6 new HTML pages
pages.forEach(page => {
  let html = baseHtml;
  
  // Remove the old AI Prompt active state
  html = html.replace('class="nav-link-sub-sub active"', 'class="nav-link-sub-sub"');
  
  // Replace the .content div completely
  const contentStart = html.indexOf('<div class="content">');
  const contentEnd = html.lastIndexOf('</div>', html.lastIndexOf('</main>')); // Just before </main>
  ...
  // Slice out the old .content and replace
  const pre = html.slice(0, contentStart);
  const post = html.slice(contentEnd); // everything from the last </div> before </main>
  
  html = pre + newContentUI + post;
  
  // Determine mapping id for the sidebar update
...
// 2. Update the sidebar in all OTHER 21 existing HTML files
...
allFiles.forEach(file => {
  ...
  content = updateSidebarInFile(content, null); // null mapping because these aren't AI Platform pages
  fs.writeFileSync(filePath, content);
  updateCount++;
});

console.log('Successfully created 6 AI Platform pages and updated ' + updateCount + ' old files.');
